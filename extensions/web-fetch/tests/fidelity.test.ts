...
async function markdownOf(name: string): Promise<string> {
	return (await extract(loadFixture(name), false)).markdown;
}
...
describe("code-regex — markdown post-processing must not rewrite code (defects 1.1, 1.2)", async () => {
	const markdown = await markdownOf("code-regex");
	...
	for (const snippet of [
		...
		it(`keeps ${JSON.stringify(snippet)} verbatim`, async () => {
			assert.ok(markdown.includes(snippet), `missing verbatim; got:\n${markdown}`);
		});
	}

	it("keeps the blank line inside the first code block", async () => {
		const [first] = fences(markdown);
		assert.ok(first, "no fenced code block");
		assert.ok(/\n\n/.test(first.code), `blank line collapsed; fence was:\n${first.code}`);
	});

	it("labels the first fence cpp from the language-cpp class", async () => {
		const [first] = fences(markdown);
		assert.ok(first, "no fenced code block");
		assert.equal(first.lang, "cpp");
	});

	it("fences both pre blocks, including the one with no code child", async () => {
		assert.equal(fences(markdown).length, 2);
	});

	it("adds no markdown escaping inside fences", async () => {
		assertNoEscapesInFences(markdown);
	});
});

describe("python-asyncio — Sphinx pre blocks (defects 1.2, 1.3)", async () => {
	const markdown = await markdownOf("python-asyncio");

	it("produces at least one fenced code block", async () => {
		assert.ok(fences(markdown).length >= 1, `fences: ${fences(markdown).length}`);
	});

	it("keeps the first code example, the one in the Sphinx sidebar", async () => {
	...
	it("keeps the REPL session verbatim", async () => {
	...
	it("keeps a language on at least one fence", async () => {
...
describe("github-readability — README code and headings (defects 1.2, 1.7)", async () => {
	const markdown = await markdownOf("github-readability");

	it("keeps the README's own heading", async () => {
		assert.ok(
	...
	it("fences all six README code blocks", async () => {
	...
	it("keeps README code lines verbatim", async () => {
...
describe("react-learn — per-line <div> code markup (defect 1.4)", async () => {
	const markdown = await markdownOf("react-learn");

	it("keeps the MyButton example on separate lines in one fence", async () => {
...
describe("div-code — per-line block markup inside <pre> (defects 1.2, 1.4)", async () => {
	const markdown = await markdownOf("div-code");

	it("fences the <div>-per-line block with its lines intact", async () => {
		assertFenceHasLines(markdown, ["line one div", "line two div", "line three div"]);
	});

	it("fences the <br>-separated block with its lines intact", async () => {
		assertFenceHasLines(markdown, ["line one br", "line two br", "line three br"]);
	});

	it("fences the <span class=line> block with its lines intact", async () => {
		assertFenceHasLines(markdown, ["line one span", "line two span", "line three span"]);
	});

	it("produces one fence per pre block", async () => {
		assert.equal(fences(markdown).length, 3);
	});
});

describe("go-tutorial — bare <pre> shell transcripts (defect 1.2)", async () => {
	const markdown = await markdownOf("go-tutorial");

	it("fences all ten pre blocks", async () => {
	...
	it("keeps the go mod init command verbatim", async () => {
...
describe("pypi-requests — doctest prompts (defect 1.2)", async () => {
	const markdown = await markdownOf("pypi-requests");

	it("puts the doctest session in a fence with unescaped prompts", async () => {
...
describe("wikipedia-transformer — section headings (defect 1.7)", async () => {
	const markdown = await markdownOf("wikipedia-transformer");
	...
	it("keeps at least eight of the ten section headings at level 2", async () => {
	...
	it("drops the [edit] section links", async () => {
		assert.ok(!markdown.includes("[edit]"), `[edit] appears ${markdown.split("[edit]").length - 1} times`);
	});

	it("leaves no anchor debris on headings", async () => {
		const debris = headings(markdown).filter((heading) => /[#¶​]$/.test(heading.text));
		assert.deepEqual(debris, []);
	});
});

describe("claude-docs-prompt-caching — Readability picked the sidebar (defect 1.6)", async () => {
	const extracted = await extract(loadFixture("claude-docs-prompt-caching"), false);

	it("returns the article, not the navigation sidebar", async () => {
	...
	it("contains the article's own content", async () => {
		assert.ok(extracted.markdown.includes("cache_control"), "cache_control never appears");
	});
});

describe("table-td-only — data table without <th> (defect 1.5)", async () => {
	const markdown = await markdownOf("table-td-only");

	it("renders a GFM table with three data rows", async () => {
		const rows = markdown.split("\n").filter((line) => line.startsWith("|"));
		// header + separator + 3 data rows
		assert.equal(rows.length, 5, `table rows: ${JSON.stringify(rows)}`);
	});

	it("keeps each row's cells together on one line", async () => {
	...
	it("keeps the caption", async () => {
		assert.ok(markdown.includes("Versions"), "caption dropped");
	});

	it("unwraps the nested single-row layout table", async () => {
...
describe("fragment — HTML with no <html> or <body> (defect 1.8)", async () => {
	const markdown = await markdownOf("fragment");

	it("converts every top-level element, not just the first", async () => {
...
describe("hn-front — layout tables and links (regression guard)", async () => {
	const markdown = await markdownOf("hn-front");

	it("emits no GFM table", async () => {
		const rows = markdown.split("\n").filter((line) => line.startsWith("|"));
		assert.deepEqual(rows, []);
	});

	it("keeps the story links", async () => {
...
describe("mdn-fetch — reference code samples (defects 1.2, 1.3)", async () => {
	const markdown = await markdownOf("mdn-fetch");

	it("keeps all five code samples fenced", async () => {
		assert.ok(fences(markdown).length >= 5, `fences: ${fences(markdown).length}`);
	});

	it("keeps the syntax block verbatim", async () => {
